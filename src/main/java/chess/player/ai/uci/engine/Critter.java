...
public class Critter extends UCIEngine {
    public Critter(Variant variant, String filePath, Option... options) throws IOException {
        super(variant, filePath, options);

        //Turn off own book since it uses .cbk format and we polygot .bin format
        waitForReady();
        sendCommand("setoption name OwnBook value false");
    }
